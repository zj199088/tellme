...
const handleWechatLogin = async () => {
  try {
    ...
    if (typeof wx !== 'undefined' && wx.login) {
      // 微信小程序环境
      const loginResult: { code: string } = await new Promise((resolve, reject) => {
        wx.login({
          success: resolve,
          fail: reject
        });
    ...
    } else if (typeof window !== 'undefined' && window.WeixinJSBridge && typeof window.WeixinJSBridge.invoke === 'function') {
      // 微信浏览器环境
      code = await new Promise((resolve, reject) => {
        window.WeixinJSBridge!.invoke('login', {}, (res: { code?: string }) => {
          if (res.code) {
            resolve(res.code);
          } else {
    ...
    error.value = '微信登录失败: ' + ((err as Error).message || '请检查网络连接');
  } finally {
    isLoading.value = false;
  }
};
