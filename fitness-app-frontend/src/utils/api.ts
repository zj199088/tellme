...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      if (!url.includes('/api/templates/list') && !url.includes('/api/categories/list') && !url.includes('/api/exercises/list') && !url.includes('/api/workout/today') && !url.includes('/api/plans/user')) {
